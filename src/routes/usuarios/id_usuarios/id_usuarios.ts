  ...
  fastify.delete(
    ...
    async function handler(req, rep) {
        ...
        fastify.log.info({
                ...
                userAgent: req.headers['user-agent'],
                ip: req.ip
              }, 'El cliente realizo la consulta DELETE para un usuario especifico.');
        ...
        fastify.log.info({ filtro: 'Id', valor: id_usuario }, 'Usuario de id ' + id_usuario + ' eliminado con exito.');
        // fastify.log.info('Fue realizada una consulta DELETE ID_USUARIOS.'); // comentario con pino-pretty
        return rep.code(204).send()
    }
  );
}
